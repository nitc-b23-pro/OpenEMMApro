...
class LlavaPythiaForCausalLM(GPTNeoXPreTrainedModel, LlavaMetaForCausalLM):
    ...
    def __init__(self, config):
        ...
        if config.action_head_type == 'act':
            ...
            class _TextHead(nn.Linear):
                """Subclassing nn.Linear (instead of wrapping one inside another module)
                keeps the saved parameter names flat: 'embed_out.weight' / 'embed_out.bias',
                matching exactly what the pretrained checkpoint has saved. A wrapper class
                that stores the Linear as self.linear renames those paths to
                'embed_out.linear.weight', which breaks weight loading -- that was the bug."""
                def forward(self, input_feature, state_tensor=None):
                    return super().forward(input_feature)

            self.embed_out = _TextHead(config.hidden_size, config.vocab_size, bias=False)
