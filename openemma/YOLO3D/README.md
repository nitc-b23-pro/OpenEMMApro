# OpenEMMA
## Setup Environment ![Python](https://img.shields.io/badge/python-3.8%20|%203.10%20|%203.12-blue)![Python 3.13 Unsupported](https://img.shields.io/badge/python-3.13%20not%20supported-red)

Run ```pip install -r openemma/YOLO3D/requirements_emma.txt```. 
## Yolo3D Inference
Use the ```yolo3d_nuScenes``` function in the ```inference.py``` file. To test the function, simple run ```python openemma/YOLO3D/inference.py```.

## Region of Interest
![roi](docs/emma_roi.svg)
